    ...
    def forward(ctx, input, mode='NONE', inplace=False, scale=1.0, blocknorm=False, blocksize=1):
        if mode == 'NONE' :
            ...
            if input.is_sparse :
                ...
                if inplace == True:
                    outputs = fpemu_hip.forward(input._values().contiguous(), mode, size, inplace, scale, blocknorm, blocksize)
                    output = input
                else :
                    outputs = fpemu_hip.forward(input._values().contiguous(), mode, size, inplace, scale, blocknorm, blocksize)
                ...
                size = input.nelement()
                outputs = fpemu_hip.forward(input.contiguous(), mode, size, inplace, scale, blocknorm, blocksize)
                output = outputs[0]
