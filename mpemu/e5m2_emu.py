...
class E5M2Emulator(object):
    ...
    def set_tensor_bindump_schedule(self, list_bindump_schedule):
        if list_bindump_schedule is not None :
            print("e5m2_emulator: Enabled binary dump for iterations : {} ".format(list_bindump_schedule))
    ...
    def enable_hw_patching(self, patch_ops):
        if patch_ops != 'NONE':
            if patch_ops in self.patchlist :
                ...
                print("e5m2_emulator: PyTorch Ops are monkey-patched to use {} kernels : {}".format(self.patch_impl, self.patch_ops))
            else :
