...
def parse():
    ...
    #parser.add_argument('--data-type', default="none", type=str, help='Supported type : bf16, bf8, bb4fp16, bb8fp16, bb4fp12, bb8fp12')
...
def main():
    ...
    if args.fp8_training:
        ...
        model, emulator = mpt_emu.initialize(model, optimizer, training_algo=args.fp8_algo, pruning_algo=args.pruning_algo,
                              list_exempt_layers=list_exempt_layers, list_layers_output_fused=list_layers_output_fused, 
                              device="cuda", verbose=True)

        if args.pruning_algo == "Unstructured":
            emulator.set_target_sparsity_weight(0.7)
            emulator.set_target_sparsity_gradient(0.7)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if emulator is not None :
            ...
            #emulator.global_steps = epoch*len(train_loader)
        # train for one epoch
...
def train(train_loader, model, criterion, optimizer, emulator, epoch, lr_scheduler):
    ...
    while input is not None:
        i += 1
        #if epoch*len(train_loader) == 165265:
        #    profiler.start()
        ...
        #if epoch*len(train_loader) == 165270:
        #    profiler.stop()
